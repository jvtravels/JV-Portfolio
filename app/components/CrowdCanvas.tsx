...
// Ported from https://skiper-ui.com/v1/skiper39 (Skiper 39 Canvas_Landing_004),
// itself adapted from https://codepen.io/zadvorsky/pen/xxwbBQV. Kept faithful to
// the original: peeps render at their native sprite-cell size, all of them walk
// at once, and resetPeep's eased offsetY is what gives the crowd its natural,
// mostly-bottom-heavy-with-a-rising-tail depth — no artificial density/scale
// knobs layered on top.
interface CrowdCanvasProps {
  src: string;
  rows?: number;
  cols?: number;
  /** Uniform scale applied to every figure's native sprite-cell size. */
  scale?: number;
}
...
export default function CrowdCanvas({ src, rows = 15, cols = 7, scale = 1 }: CrowdCanvasProps) {
  ...
  useEffect(() => {
    ...
    // Native sprite-cell figures (~240x324px) are the right scale for a wide
    // desktop strip but overwhelm a narrow viewport, showing only a few heads
    // cropped oddly. Scale down once at mount based on viewport width — the
    // upstream algorithm otherwise stays untouched.
    ...
    const createPeep = ({ image, rect }: { image: HTMLImageElement; rect: number[] }): Peep => {
      const peep: Peep = {
        ...
        setRect(r) {
          peep.rect = r;
          peep.width = r[2] * effectiveScale;
          peep.height = r[3] * effectiveScale;
        },
        render(context) {
          context.save();
